...
sub LGTV_WebOS_TimerStatusRequest($) {
    ...
    if( !IsDisabled($name) and $hash->{CD} and $hash->{helper}{device}{registered} == 1 ) {
        ...
        if( AttrVal($name,'pingPresence', 0) == 0 or ReadingsVal($name,'presence','absent') ne 'absent' ) {
...
sub LGTV_WebOS_Open($) {
    ...
    if( AttrVal($name,'pingPresence', 0) == 0 or ReadingsVal($name,'presence','absent') ne 'absent' ) {
		...
		Log3 $name, 4, "LGTV_WebOS ($name) - absent";
    }
}
...
sub LGTV_WebOS_WakeUp_Udp($@) {
    ...
    my $name  = $hash->{NAME};
    Log3 $name, 3, "Sub LGTV_WebOS_WakeUp_Udp ($name) - start WOL socket";
	...
	# davon ausgehen, dass es funktioniert, sonst wird der nächste Test wieder auf 'absent' ändern
